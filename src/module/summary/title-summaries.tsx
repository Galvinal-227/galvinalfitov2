...
const TitleSummaries = ({ text, observeId }: { text: string; observeId: string }) => {
  ...
  return (
    <div className="relative mb-[5vh] w-fit" style={{ borderBottom: '2px solid white' }}>
      <h2 className="font-pixel text-4xl font-semibold underline opacity-0">{text}</h2>
      <StaggerElementFooter
        ...
        perLetter
        tag="h2"
        className="pointer-events-none absolute left-1/2 top-1/2 z-10 m-0 -translate-x-1/2 -translate-y-1/2 whitespace-nowrap font-pixel text-4xl font-medium leading-[1.3] text-white"
      >
        {text}
      </StaggerElementFooter>
      <StaggerElementFooter
